...
(function () {
    ...
    const PLAYER_W = 40;
    const PLAYER_H = 40;
    const PLAYER_VISUAL_OFFSET_Y = 10; // 스프라이트 하단 여백 보정
    // 물리 상수 (120fps 기준)
    // 속도(px/tick) = 60fps값 ÷ 2  |  가속도(px/tick²) = 60fps값 ÷ 4
    const GRAVITY = 0.25;
    const JUMP_FORCE = -7.5;
    const INITIAL_SPEED = 2.5;
    const MAX_SPEED = 10;
    const SPEED_INCREMENT = 0.00075;
    const OBSTACLE_MIN_GAP = 200; // 넓힘 (초기 속도 기준 점프 후 착지 시간 확보)
    const OBSTACLE_MAX_GAP = 280; // 넓힘
    ...
    // ========== Difficulty Scaling (Phase System) ==========
    function getDifficultyParams(currentScore) {
        let gapMultiplier = 1.0;
        let obstacleHeightBonus = 0;
        let obstacleWidthBonus = 0;
        let doubleObstacleChance = 0;

        if (currentScore >= 700) {
            // Phase 3: 극한 난이도 — 간격 급축소, 대형 장애물, 연속 장애물 빈번
            const t = Math.min((currentScore - 700) / 300, 1); // 0→1 over 700→1000
            gapMultiplier = 0.7 - t * 0.15;       // 0.7 → 0.55
            obstacleHeightBonus = 15 + t * 10;     // +15 → +25
            obstacleWidthBonus = 5 + t * 5;        // +5 → +10
            doubleObstacleChance = 0.3 + t * 0.2;  // 30% → 50%
        } else if (currentScore >= 400) {
            // Phase 2: 점진적 강화 — 간격 줄어들고 장애물 커지기 시작
            const t = (currentScore - 400) / 300;  // 0→1 over 400→700
            gapMultiplier = 1.0 - t * 0.3;         // 1.0 → 0.7
            obstacleHeightBonus = t * 15;           // 0 → +15
            obstacleWidthBonus = t * 5;             // 0 → +5
            doubleObstacleChance = t * 0.3;         // 0% → 30%
        }
        // Phase 1 (0~400): 기본값 유지 — 초보자 접근성 보장

        return { gapMultiplier, obstacleHeightBonus, obstacleWidthBonus, doubleObstacleChance };
    }
    ...
    // 고정 타임스텝 관련
    let lastTimestamp = 0;
    let accumulator = 0;         // 시간 누적기 (ms)
    let elapsedTicks = 0;        // 누적 틱 수 (속도 증가용)
    let scoreAccumulator = 0;    // 틱 누적 (점수 계산용)
    const MAX_ACCUMULATOR = FRAME_MS * 10; // 최대 누적량 (탭 복귀 시 폭주 방지)
    ...
    function spawnObstacle() {
        const diff = getDifficultyParams(score);
        const h = 25 + Math.random() * 25 + diff.obstacleHeightBonus;
        const w = 15 + Math.random() * 15 + diff.obstacleWidthBonus;
        obstacles.push({
            x: CANVAS_W + 10,
            y: GROUND_Y - h,
            w: w,
        ...
        // 연속 장애물 (더블 장애물) — 고득점 구간에서 확률적으로 등장
        if (Math.random() < diff.doubleObstacleChance) {
            const h2 = 25 + Math.random() * 20 + diff.obstacleHeightBonus * 0.7;
            const w2 = 15 + Math.random() * 10;
            const innerGap = 60 + Math.random() * 40; // 60~100px 간격 (한 번의 점프로 둘 다 넘기기 어려움)
            obstacles.push({
                x: CANVAS_W + 10 + w + innerGap,
                y: GROUND_Y - h2,
                w: w2,
                h: h2,
            });
        ...
        // 속도에 따른 간격 — sqrt 스케일링 + Phase 보정
        // 기존: 선형 비례 → 고속에서 간격이 캔버스보다 커져서 난이도 정체
        // 수정: sqrt 비례 → 고속에서도 적절한 간격 유지
        const speedFactor = Math.max(1, speed / INITIAL_SPEED);
        const sqrtFactor = Math.sqrt(speedFactor);
        const dynamicMinGap = OBSTACLE_MIN_GAP * sqrtFactor * diff.gapMultiplier;
        const dynamicMaxGap = OBSTACLE_MAX_GAP * sqrtFactor * diff.gapMultiplier;
        nextObstacleDistance = dynamicMinGap + Math.random() * (dynamicMaxGap - dynamicMinGap);
    }
    ...
    function update() {
        ...
        // Speed up — 틱 기반
        elapsedTicks++;
        speed = Math.min(MAX_SPEED, INITIAL_SPEED + elapsedTicks * SPEED_INCREMENT);

        // Player physics — 고정 타임스텝이므로 dt 불필요
        player.vy += GRAVITY;
        player.y += player.vy;
    ...
    function draw() {
        ...
        if (playerEl) {
            ...
            playerEl.style.top = ((player.y + PLAYER_VISUAL_OFFSET_Y) * canvasScale.y) + 'px';
    ...
    function gameLoop(timestamp) {
        ...
        // 고정 간격(FRAME_MS)마다 물리 업데이트 실행
        while (accumulator >= FRAME_MS) {
            update();
            accumulator -= FRAME_MS;
            if (gameState !== 'running') return; // update 중 게임오버 시 탈출
        }
    ...
    function showGameOverNormal() {
        ...
        overlaySubtitle.innerHTML = `점수: <strong>${score}</strong> | 최고: <strong>${highScore}</strong><br><span style="font-size:0.85rem; color:#aaa;">다시 도전하려면 화면 터치 또는 스페이스바</span>`;
    ...
    function showGameOverWithSubmit() {
        ...
        const doSubmit = () => {
            ...
            setTimeout(() => {
                ...
                btn.remove();
                overlayTitle.textContent = 'GAME OVER';
                overlaySubtitle.innerHTML = `점수: <strong>${finalScore}</strong> | 최고: <strong>${highScore}</strong><br><span style="font-size:0.85rem; color:#aaa;">다시 도전하려면 화면 터치 또는 스페이스바</span>`;
            }, 1200);
        };
    ...
    document.addEventListener('keyup', (e) => {
        ...
        if (e.code === 'Space' || e.key === ' ' || e.key === 'ArrowUp') {
            e.preventDefault();
            // 상승 중에 키를 떼면 추진력을 줄임 (가변 점프)
            if (gameState === 'running' && player.jumping && player.vy < 0) {
                player.vy *= 0.5;
            }
        }
    });
    ...
    if (canvas) {
        ...
        if (container) {
            ...
            container.addEventListener('touchend', (e) => {
                if (window._isMinigameOpen && gameState === 'running' && player.jumping && player.vy < 0) {
                    player.vy *= 0.5; // 가변 점프
                }
            });
            ...
            container.addEventListener('click', (e) => {
                if (window._isMinigameOpen && e.target !== document.querySelector('.game-nickname-input')) {
                    if (gameState === 'running' && player.jumping) return; // 빠른 낙하에 소모됨
                    handleGameInput(e);
                }
            });
        }
    ...
    if (overlay) {
        ...
        overlay.addEventListener('click', (e) => {
            if (window._isMinigameOpen && !e.target.classList.contains('game-nickname-input') && !e.target.classList.contains('game-submit-btn')) {
                if (gameState === 'running' && player.jumping) return;
                handleGameInput(e);
            }
        });
    }
    ...
    window.openMiniGame = function () {
        ...
        // showModal() 이후에 캔버스 크기 계산 (==> dialog가 화면에 렌더링된 후여야 getBoundingClientRect가 정확함)
        resizeCanvas();
        resetGame();
